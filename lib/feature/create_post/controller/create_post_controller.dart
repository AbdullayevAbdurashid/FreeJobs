...
class CreatePostController extends GetxController implements GetxService{
  ...
  Future<void> createCustomPost(String schedule) async {
    _isLoading = true;
    update();
  ...
  Future<void> getProvidersOfferList(int offset,String postId,{reload = false}) async {
    ...
    print(providerOfferModel!.message);
  ...
  Future<Response> updatePostStatus(String postId, String providerId, String status) async {
    Response response = await createPostRepo.updatePostStatus(postId, providerId, status);

    return response;
  }

  Future<Response> makePaymentWithWalletMoney(String postId, String providerId, String paymentMethod) async {
    ...
    Response response = await createPostRepo.makePaymentWithWalletMoney(
