...
import 'package:demandium/feature/checkout/widget/custom_post/payment_method.dart';
...
class _CustomPostCheckoutScreenState extends State<CustomPostCheckoutScreen> {
  ...
  void initState() {
    ...
    Get.find<CheckOutController>()
        .updateSelectedDigitalPaymentIndex(-1, shouldUpdate: false);
  ...
  Widget build(BuildContext context) {
    if (_configModel.content?.walletStatus == 0 &&
        _configModel.content?.cashAfterService == 0 &&
        _configModel.content?.digitalPayment == 0) {
      ...
      body: GetBuilder<CheckOutController>(builder: (checkoutController) {
        if (checkoutController.postDetails != null) {
          ...
          Get.find<LocationController>().updateSelectedAddress(
              checkoutController.postDetails?.serviceAddress);
                ...
                child: Column(
                  children: [
                    ...
                    if (checkoutController
                        .postDetails!.additionInstructions!.isNotEmpty)
                      DescriptionExpansionTile(
                    ...
                    SizedBox(
                      height: ResponsiveHelper.isDesktop(context) ? 70 : 110,
                    )
                  ],
                ),
              ));
    ...
    if (Get.find<AuthController>().acceptTerms) {
      if (checkOutController.selectedPaymentMethod == PaymentMethodName.cos) {
        ...
        Response response = await Get.find<CreatePostController>()
            .updatePostStatus(widget.postId, widget.providerId, 'accept');
        Get.back();
        if (response.statusCode == 200 &&
            response.body['response_code'] == "default_update_200") {
          Get.offNamed(RouteHelper.getBookingScreenRoute(false));
        } else {
          Get.offNamed(RouteHelper.getBookingScreenRoute(false));
        }
        ...
        Response response = await Get.find<CreatePostController>()
            .makePaymentWithWalletMoney(
                widget.postId, widget.providerId, "wallet_payment");
        Get.back();
        ...
        if (response.statusCode == 200 &&
          ...
          Get.offNamed(RouteHelper.getOrderSuccessRoute('success'));
            ...
            response.body["response_code"] ==
                "insufficient_wallet_balance_400") {
        ...
        if (checkOutController.selectedDigitalPayment == -1) {
          ...
          String? addressID = addressModel != null
          ...
          if (addressID != '') {
            ...
            url =
                '${AppConstants.baseUrl}/payment/${paymentGateways![checkOutController.selectedDigitalPayment].replaceAll('_', '-')}/pay?access_token=${base64Url.encode(utf8.encode(userId))}'
            ...
            if (GetPlatform.isWeb) {
              url =
                  '$url&&callback=$protocol//$hostname:$port${RouteHelper.orderSuccess}';
