...
import 'package:demandium/feature/checkout/widget/payment_section/custom_post_payment_method_button.dart';
...
class _PaymentMethodState extends State<PaymentMethod> {
  ...
  List<CustomPostPaymentMethodButton> listOfButton =[
    if(Get.find<SplashController>().configModel.content?.digitalPayment==1)
      CustomPostPaymentMethodButton(title: "pay_now".tr,paymentMethodName: PaymentMethodName.digitalPayment, assetName: Images.pay,),
    if(Get.find<SplashController>().configModel.content?.cashAfterService==1)
      CustomPostPaymentMethodButton(title: "cash_after_service".tr,paymentMethodName: PaymentMethodName.cos,assetName: Images.cod,),
    if(Get.find<SplashController>().configModel.content?.walletStatus==1)
     CustomPostPaymentMethodButton(title: "wallet_money".tr,paymentMethodName: PaymentMethodName.walletMoney,assetName: Images.walletMenu,),
  ];
          ...
          Padding(padding: EdgeInsets.symmetric(horizontal: Get.width*padding),
            child: GridView.builder(
              ...
              gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                ...
                mainAxisExtent: ResponsiveHelper.isMobile(context)? 50: 60,
                  ...
                  itemBuilder: (context, index)  {
                    return GetBuilder<CheckOutController>(builder: (controller){
                      return GestureDetector(
                        onTap: () => controller.updateSelectedDigitalPaymentIndex(index),
                        child: Stack(children: [
                          DigitalPayment(
                            paymentGateway: paymentGateways[index],
                            addressId: Get.find<LocationController>().selectedAddress?.id.toString(),
                            redirectDirectlyPaymentScreen: false,
                          ),

                         controller.selectedDigitalPayment==index?
